...
/* THE SIX STAGES. Two names are what a stage is called and what its artifact
   ...
   drives. Docs is a view a person reads beside the model: an HTML file at
   docs/<model>.html, named for the model it draws. A member repository maps
   the last four onto its own tree, and the profile line of its campaign's
   kind says which of them it has at all. */
abstract sig Stage {}
