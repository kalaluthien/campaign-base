...
prefilter, the cuts, the `loss` each wrong word costs and the TIER -- and a reader asks by group name. **A cut is fitted, not typed**: `campaign-jev.py fit --write` derives it from the labelled raw answers and that `loss`, and writes `unmeasurable` with its counts where they are too few. The tier is what an answer may do: `shadow` logs only, `advise` prints,
