...
`/close` (`.claude/skills/close/SKILL.md`): its one call is
`scripts/campaign-close.py <target>`, which reads the scope off the target. Run
that and stop, or the readings below take the close for a sub-issue.
...
prefilter, the cuts, the `loss` each wrong word costs and the TIER -- and a reader asks by group name. **A cut is fitted where it can be measured**: `campaign-jev.py fit --write` derives it from the labelled raw answers and that `loss`; it writes `unmeasurable` with its counts, and the typed cut stands, on any of five: no labelled raw answer, a class under the floor, no `loss` whose `wrong_yes` and `wrong_no` are both numbers above zero, a fitted cut doing no better held out than one word always, or two edges that meet. The tier is what an answer may do: `shadow` logs only, `advise` prints,
...
the three ways round that script -- a `herdr agent prompt` or a `herdr pane
send-text` carrying the assignment sentence, and a launch line carrying a
prompt -- since its four launch checks hold on its own path and nowhere else;
what it reads as a prompt on that line, and what that reading costs, is its
own docstring's. The full procedure — the launch
...
travel as one. **Assigning a sub-issue to a session is
`scripts/campaign-assign.py <pane> <sub-issue>`**, a delegate's *first* brief
included with `--assume-fresh` (§ Delegate launch), and what it refuses and why
is its docstring's.
...
over who wrote what, and `unverified-done` over the round's `REPORT` — does it
claim a thing done with no check that could have failed named beside it. Each is at `shadow`;
